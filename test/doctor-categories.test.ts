 ...
 *
 * Reads src/commands/doctor.ts plus imported doctor check modules via a
...
describe('doctor-categories drift guard', () => {
  test('every check name in doctor.ts source belongs to exactly one category set', () => {
    ...
    if (missing.length > 0) {
      throw new Error(
        `These check names appear in doctor check sources but are not categorized in ` +
  ...
  test('every categorized name is currently used in doctor check sources (no stale entries)', () => {
    const discovered = enumerateCheckNames();
    const allCategorized = new Set<string>([
      ...BRAIN_CHECK_NAMES,
      ...SKILL_CHECK_NAMES,
